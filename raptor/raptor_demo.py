...
class SummarizationModel(BaseSummarizationModel):
    def __init__(self, model_name="meta-llama/Meta-Llama-3-8B-Instruct"):
        ...
        self.summarization_pipeline = pipeline(
            ...
            device = torch.device("cuda:1,3,4" if torch.cuda.is_available() else "cpu"),  # Use "cpu" if CUDA is not available
        )
...
class QAModel(BaseQAModel):
    def __init__(self, model_name= "intfloat/e5-base-v2"):
        ...
        self.qa_pipeline = pipeline(
            ...
            model=model_name,
            model_kwargs={"torch_dtype": torch.bfloat16},
            device = torch.device("cuda:1,3,4" if torch.cuda.is_available() else "cpu"),
        )
